...
export class LoginVlad extends BasePage {
  ...
  async apiLogin(username: string, password: string): Promise<{ token: string, userId: string }> {
    if (!process.env.API_BASE_URL) {
      throw new Error('API_BASE_URL is not defined in .env');
    }

    const response = await this.page.request.get(`${process.env.API_BASE_URL}/api/login`, {
    ...
    if (!responseBody.token || !responseBody.userId) {
