...
export class CSRSearchPage {
    ...
    async verifyItemVisible(itemName: string): Promise<void> {
        try {
            ...
            await this.page.screenshot({ path: 'item_not_found.png' });
            throw new Error(`Item "${itemName}" not found in iframe: ${error.message}`);
        }
    }
    ...
    async verifyItemNotVisible(itemName: string): Promise<void> {
        try {
            ...
            await this.page.screenshot({ path: 'item_still_visible.png' });
            throw error;
        }
    }
