...
readonly savingIndicator = this.page.locator('.saving-indicator');
  ...
  async saveAndHandleDialog(): Promise<void> {
    ...
    try {
        await this.notificationDialog.waitFor({ state: 'visible', timeout: 7000 });
        await this.click(this.updateButton, { timeout: 5000 });
        console.log('Closed notification dialog');
    } catch (error) {
        console.log('Notification dialog did not appear');
    }

    // 5. Явное ожидание завершения сохранения
    await this.page.waitForFunction(() => {
        return document.querySelector('.saving-indicator') === null;
    }, { timeout: 20000 });

    // 6. Проверка успешности сохранения
    await this.expectToBeVisible(this.successNotification, 10000)
        .catch(() => { throw new Error('Save failed - no success notification'); });
}
